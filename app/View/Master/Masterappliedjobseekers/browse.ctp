<div class="x_panel">
	...
	<?php
		$occ_app = $occ['OccupationApply']['id'];
		$status = $occ['OccupationApply']['status'];
	?>
	<div class="x_content" id="beginning_point">
		<table class="table-st">
			<tr>
				<td colspan="3" style="background-color: black;height: 60px;margin-bottom: 100px;padding: 10px;">
					<div class="row tex-center">
						<div class="col-md-3">
							<?php if ($company_info['CmpHeadhunter']['avaliable_mail'] != 0) : ?>
								<button href="#myModal" id="contact" data-toggle="modal" class="btn btn-primary btn-lg btn-blocky full-width">Contact</button>
							<?php else: ?>
								<button href="#secondModal" id="contact" data-toggle="modal" class="btn btn-primary btn-lg btn-blocky full-width">Contact</button>
							<?php endif; ?>
						</div>

						<div class="col-md-3">
							<button id="adoption" class="btn btn-warning btn-lg btn-blocky full-width">Adoption</button>
						</div>
						<div class="col-md-3">
						<button id="successAdoption" class="btn btn-success btn-lg btn-blocky full-width">Successfull Adoption</button>
						</div>
						<div class="col-md-3">
						<button id="notAdopted" class="btn btn-default btn-lg btn-blocky full-width">Not adopted</button>
						</div>
					</div>
				</td>
			</tr>
		...
		<table class="table-st">
			<tr>
				<td colspan="3" class="labelbg">
					<div class="col-md-10 col-sm-10 col-xs-12" style="margin-left: -40%;">
	...
	<div class="x_content">
		<table class="table-st">
										...
										<?php if (!empty($user_edu)): ?>
											...
											<div class="education col-md-12 col-sm-12 col-xs-12 none_padding" style="line-height: 2.5em;margin-bottom: -3%;margin-left: 2%;">
												...
												<div class="col-md-12 col-sm-12 col-xs-12" style="word-break: break-all;margin-top: 0%;">
													<h3 class="career-hist-title " style="font-size: 17px;margin-bottom: 0%;">
							...
							<?php if (!empty($user_computing)): ?>
								...
								<div class="col-md-12 col-sm-12 col-xs-12 none_padding">
									<div class="col-md-11 col-sm-12 col-xs-12 title-cmp" style="margin-top: -2%;">
										<h3 class="career-hist-title" style="margin-top: 3%;font-size: 17px;margin-bottom: 1%;">Microsoft Office</h3>
									</div>
								</div>
			...
			<tr>
				<body>
					<section style="border: none;">
						<div class = "x_content" style=" margin-bottom: 2%;">
							<table class = "table-st">
								<tr>
									<td colspan="3" class="labelbg" style="padding-left: 2%;">
										<div class="col-md-10 col-sm-10 col-xs-12">
											<?php echo "<label class='main-label'>Curriculum Vitae</label>"; ?>
										</div>
									</td>
								</tr>
							</table>
			...
			<tr>
				<td colspan="3">
					<div class="form-group" style="border-bottom: 2px solid #808080;padding-bottom: 24px;">
						<center>
							<?php echo $this->Html->link('Download', array('controller' => 'masterappliedjobseekers', 'action' => 'profilePdf', 'ext' => 'pdf', $occ['User']['id']), array('class' => 'btn btn-default btn-sm','style' => "background-color: #DCDCDC;color: slategrey;width: 20%;")); ?>
						</center>
					</div>
				</td>
			</tr>
			...
			<tr>
				<td colspan="3">
					<div class="form-group" id="contact" style="margin-top: 21px;border-bottom: none;margin-bottom: 1px;">
						<center>
				...
				<div class="modal-content">
						...
						<div class="modal-body">
								<?php echo $this->Form->create('Message', array(
									'url' => array(
										'controller' => 'Masterappliedjobseekers',
										'action' => 'contact'
									),
									...
									'onsubmit' => "confirm('Do you send the message to the jobseeker? If you sent the message, 1 of the remaining number of  message transmission authority will decrease.');"
										...
										<?php echo $this->Form->input('occupation_id', array('type' => 'hidden', 'value' => $occ_app));?>
						...
						<div class="modal-body">
								<?php echo $this->Form->create('Message', array(
									...
									'onsubmit' => "confirm('Do you send the message to the jobseeker? If you sent the message, 1 of the remaining number of  message transmission authority will decrease.');"
										...
										<?php echo $this->Form->input('user', array('type' => 'hidden', 'value' => $occ['OccupationApply']['id'])); ?>
...
<script type="text/javascript">
	var occ_app = <?php echo $occ_app; ?>;
	var status = <?php echo $status; ?>;
	$(document).ready(function(){
		if(status == 1){

			// simple datas of this jobseeker
			$('#adoption').attr('disabled', 'disabled');
			$('#notAdopted').attr('disabled', 'disabled');
			$('#successAdoption').attr('disabled', 'disabled');


		}else if(status == 2){

			//had been maked contact of this jobseeker
			$('#adoption').removeAttr('disabled');
			$('#successAdoption').attr('disabled', 'disabled');
			$('#notAdopted').removeAttr('disabled');
			...
			$('#adoption').attr('disabled', 'disabled');
			$('#notAdopted').attr('disabled', 'disabled');
			$('#successAdoption').removeAttr('disabled');
			$('#contact').attr('disabled', 'disabled');
	...
	$(document).on('click', '#successAdoption', function (){
		var choice = confirm("Are you sure?");
		if (choice) {
			$.ajax({
				url: "/master/appliedjobseeker/successAdoption",
				type: "POST",
				data:{id : occ_app},
				dataType: "json",
	...
	$(document).on('click', '#adoption', function (){
		...
		if (choice) {
			$.ajax({
				url: "/master/appliedjobseeker/adoption",
				type: "POST",
				data:{id : occ_app},
				dataType: "json",
				success : function(response){
					$('#adoption').attr('disabled', 'disabled');
					$('#successAdoption').removeAttr('disabled', 'disabled')
					$('#notAdopted').attr('disabled', 'disabled');
					$('#contact').attr('disabled', 'disabled');
				},
				error: function(){}
	...
	$(document).on('click', '#notAdopted', function (){
		var choice = confirm("Are you sure?");
		if (choice) {
			$.ajax({
				url: "/master/appliedjobseeker/not_adopted",
				type: "POST",
				data:{id : occ_app},
				dataType: "json",
				success : function(response){
